...
for name, classifier in classifiers.items():
    ...
    means[name] = cv_scores.mean()
    stds[name] = cv_scores.std()
...
plt.bar(accuracy_test.keys(), accuracy_test.values(), color='purple', label='Accuracy')

# Aggiunta della media e della deviazione standard della cross-validation
for i, name in enumerate(accuracy_test.keys()):
    plt.errorbar(i, cv_means[name], yerr=cv_stds[name], fmt='o', color='green', label='Mean ± Std' if i == 0 else "")
...
for name in classifiers.keys():
    ...
    print(f"  Mean Accuracy: {cv_means[name]:.4f}")
    print(f"  Std Dev: {cv_stds[name]:.4f}")
    print("-" * 40)
